...
if filter_name == 'F410M':    
    rootDIR_noVar = f'../../Data/F410M_LIKELY_Th8/simulated_data/Sep_540_PA_90_Flux_1450000_P_8.50_VAR0.00_DXY0.0_PHASE-2.734'
    rootDIR = f'../../Data/F410M_LIKELY_Th8/simulated_data/Sep_540_PA_90_Flux_1450000_P_8.50_VAR5.00_DXY0.0_PHASE-2.734'  
    
    import h5py
    import matplotlib.pyplot as plt

    h5 = h5py.File(
        rootDIR + "/lightcurves/Planet_LightCurves_roll_0_aper_2.0.hdf5",
        "r"
    )
    h52 = h5py.File(
        rootDIR + "/lightcurves/Planet_LightCurves_roll_1_aper_2.0.hdf5",
        "r"
    )
    print(list(h5.keys()))
    ...
    inj = 9200 * (
    1 + 0.05*np.sin(2*np.pi*t/8.5 + -2.734)
    )

    plt.figure(figsize=(7,3))
    plt.plot(t, raw, ".-", label="measured aperture")
    plt.plot(t, inj, "--", lw=2, label="Expected injected")
    plt.xlabel("Time [hr]")
    plt.ylabel("Raw aperture flux")
    plt.title("Injected raw light curve")
    plt.grid(alpha=0.3)
...
time0, lc_Var0 = np.loadtxt(os.path.join(rootDIR, 'lightcurves', 'Planet_LightCurve_Corrected_Roll_0.txt'), unpack=True)
time1, lc_Var1 = np.loadtxt(os.path.join(rootDIR, 'lightcurves', 'Planet_LightCurve_Corrected_Roll_1.txt'), unpack=True)
...
print("Difference from A=0:")
print("final:", np.std(lc_Var - lc_noVar))

print("Max difference from A=0:")
print("final:", np.max(np.abs(lc_Var - lc_noVar)))
...
#    axes[0].plot(rebin(t_hr1, 20), rebin(lc_noVar1, 20), color='C0', ds='steps-mid')
    ...
    axes[0].set_title(f'Injection/Recovery at PA=210° -- {filter_name}', weight='bold')
    axes[0].set_ylim([0.97, 1.03])
    axes[1].set_ylim([0.97, 1.03])
    axes[0].set_ylabel(' ')
    fig.tight_layout()
    if saveFigure:
        plt.savefig(f'../../VizPlots/Injected_LightCurves_PA_{plot_PA}_{filter_name}.pdf', bbox_inches='tight')

#chisq_noVar, _, _, _ = test_deviation_from_flat(lc_noVar, lc_noise_noVar)
#chisq_non_variable_amplitudes[i] = chisq_noVar

chisq, _, _, _ = test_deviation_from_flat(lc_Var, lc_noise_Var)

print(chisq)


deltaBIC = delta_bic_flat_vs_sinusoid(t_hr, lc_Var, lc_noise_Var, amp=5.0, period=8.5)
...
fig.tight_layout()
if saveFigure:
    plt.savefig(f'../../VizPlots/InjectionRecoverySignificance_PA_{plot_PA}_{filter_name}.pdf', bbox_inches='tight')
